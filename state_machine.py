"""State machine — wake word + command flow + GPIO relay control."""
...
class StateMachine:
    ...
    def _execute_on(self):
        if self.lamp_is_on:
            ...
            print(f"[fsm] Lamp ON (GPIO {RELAY_PIN} -> LOW, relay energized)")
    ...
    def _execute_off(self):
        if not self.lamp_is_on:
            ...
            print(f"[fsm] Lamp OFF (GPIO {RELAY_PIN} -> HIGH, relay released)")
            self.lamp_is_on = False
            GPIO.output(RELAY_PIN, RELAY_INACTIVE)
